...
**Hosts** ana dizininde üst klasörler görünür; gruba atanmamış hostlar **Ungrouped** klasöründedir. Bir grup klasörüne tek tıklayınca doğrudan o gruptaki hostlar ve alt klasörler açılır. Sol menüdeki klasörler tek tıkla gezinme içindir. Üstteki klasör yolu ile geri dönün. Arama açık klasörün hostlarını arar. Yeni host, açık gruba eklenir.

## Kartlar, bilgiler ve sağ tık menüsü

Host, kimlik, snippet, tünel, workspace, known host veya log kaydına **tek tıklayınca** sağda salt okunur bilgiler açılır. Hostlarda hızlı çift tıklamayı ayırmak için bilgiler yaklaşık yarım saniye sonra açılır; ikinci tık gelirse bekleme iptal edilir ve bağlantı başlatılır. Klasör tek tıkla açılır; klasör bilgileri **sağ tık → Bilgiler** üzerinden görüntülenir. Parola ve özel anahtar içeriği bu panelde gösterilmez. Düzenlemek için **sağ tık → Edit** kullanın. **Ctrl/Command + tık** ayrı kayıtlar, **Shift + tık** aralık seçer; seçim kutucuğu veya üstte seçime özel işlem çubuğu yoktur. Klavyeyle kart odaktayken **Shift+F10** menüyü açar; oklar, Home/End, Enter ve Escape kullanılabilir.
